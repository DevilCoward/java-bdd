...
public class Timedelta implements Comparable<Timedelta> {
    ...
    public boolean equals(Object other) {
        return (other instanceof Timedelta)
                && (Math.abs(((Timedelta) other).asMilliSeconds())
                        == Math.abs(this.asMilliSeconds()));
    }
    ...
    public String toString() {
        ...
        if (hours != 0) {
            return String.format(
                    (this.milliSeconds < 0 ? "- " : "") + "%02d h:%02d m:%02d s::%d ms",
                    Math.abs(hours),
                    Math.abs(minutes),
                    Math.abs(seconds),
            ...
            return String.format(
                    (this.milliSeconds < 0 ? "- " : "") + "%02d m:%02d s::%d ms",
                    Math.abs(minutes),
                    Math.abs(seconds),
                    Math.abs(milliSeconds));
        } else if (seconds != 0) {
            return String.format(
                    (this.milliSeconds < 0 ? "- " : "") + "%02d s::%d ms",
                    Math.abs(seconds),
                    Math.abs(milliSeconds));
        }
